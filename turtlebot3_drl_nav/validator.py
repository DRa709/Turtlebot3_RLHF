...
def _i(value: str) -> Optional[int]:
    if value == "":
        return None
    # Seeds and episode keys are recorded as decimal integers and may use the
    # full signed 63-bit range.  A float round trip loses precision above
    # 2**53, so validate the lexical form and parse it directly.
...
def expected_epsilon(algorithm: str, params: Dict[str, object], env_step: int) -> Optional[float]:
    """Schedule value used for the action taken at ``env_step`` (counted from 0)."""
    if algorithm not in ("DQN", "DoubleDQN", "DuelingDoubleDQN"):
        return None
    start, end, decay = float(params["epsilon_start"]), float(params["epsilon_end"]), int(params["epsilon_decay_steps"])
    progress = min(1.0, (env_step - 1) / max(1, decay))
...
def _validate_transitions(report, rows, commands, common, control_period, hold_tol, validation, budget, phase_type, algorithm) -> None:
    ...
    training_mode = "noisy" if algorithm == "RainbowDQN" else ("stochastic" if algorithm == "DiscreteSAC" else "epsilon_greedy")
    context_ok = all(
...
def _validate_updates(report, updates, algorithm, params, budget) -> None:
    ...
    eps_ok = True
    for u in updates:
        expected = expected_epsilon(algorithm, params, _i(u["env_step"]))
        if expected is not None and abs(expected - _f(u["epsilon"])) > 1e-9:
            eps_ok = False
    report.check(eps_ok, "logged epsilon equals the schedule value used for the action at that env_step")
...
def _validate_evaluation_posthoc(report, evaluation, scenarios, protocol, phase, common, transitions, run_dir, algorithm, sampler) -> None:
    ...
    expected_modes = ["greedy"] if algorithm in ("DQN", "DoubleDQN", "DuelingDoubleDQN", "RainbowDQN") else ["deterministic", "stochastic"]
