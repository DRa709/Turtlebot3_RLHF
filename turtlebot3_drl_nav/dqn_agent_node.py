"""ROS 2 (Foxy) DQN agent node. Algorithm-specific file.

All scheduling lives in the shared :mod:`orchestrator`; the learner is
:class:`dqn.DQNAgent`. This node converts messages, executes effects, records
...
from .dqn import ALGORITHM, DQNAgent, DQNConfig, GreedyPolicy, _torch_load, parameter_digest, validate_checkpoint_payload
...
EXIT_FATAL = 3
EXIT_INTERRUPTED = 4
AGENT_CONFIG_NAME = "phase1_dqn.yaml"
NODE_NAME = "dqn_agent"
...
class DQNAgentNode(Node):
    def __init__(self) -> None:
        ...
        if str(self.identity["algorithm_version"]) != str(params["algorithm_version"]):
            raise ValueError("algorithm_version differs between run identity and DQN configuration")
        ...
        self.learner = DQNAgent(OBSERVATION_DIM, len(commands), learner_config, self.device, learning_seed)
    ...
    def _housekeeping(self) -> None:
        if self.interrupt_requested and self.finished_reason is None:
            self.interrupt_requested = False
            path = os.path.join(self.run_dir, "checkpoints", f"dqn_seed{self.learner.seed}_step{self.learner.env_steps}_interrupted_full.pt")
            try:
...
AGENT_NODE_CLASS = DQNAgentNode


def main(args=None) -> None:
    rclpy.init(args=args)
    node: Optional[DQNAgentNode] = None
    exit_code = 0
    try:
        node = DQNAgentNode()
        rclpy.spin(node)
