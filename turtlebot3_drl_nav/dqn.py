"""Phase-1 DQN learner (roadmap §3.1). Algorithm-specific file; no ROS dependency.

Update rule, verified bit-exactly against an independent recomputation:
    y_i = r_i + gamma * (1 - m_i^term) * max_a' Q_target(o'_i, a')
...
class DQNConfig:
    ...
    def validate(self) -> None:
        ...
        if not all(np.isfinite(float(value)) for value in scalar_values):
            raise ValueError("DQN scalar hyperparameters must be finite")
        integer_values = (
        ...
        if any(isinstance(value, bool) or not isinstance(value, int) for value in integer_values):
            raise ValueError("DQN step counts and sizes must be integers")
...
def validate_checkpoint_payload(payload: Dict, allowed_kinds: tuple = ("full", "policy_only")) -> DQNConfig:
...
class DQNAgent:
    def __init__(self, observation_dim: int, action_dim: int, config: DQNConfig, device: torch.device, seed: int) -> None:
        config.validate()
    ...
    def optimize(self) -> Optional[Dict[str, float]]:
        ...
        if not torch.isfinite(predictions).all() or not torch.isfinite(targets).all() or not torch.isfinite(loss):
            raise RuntimeError("DQN update produced a non-finite prediction, target, or loss")
        self.optimizer.zero_grad()
        loss.backward()
        grad_norm = nn.utils.clip_grad_norm_(self.online.parameters(), self.config.gradient_clip_norm)
        if not torch.isfinite(grad_norm):
            self.optimizer.zero_grad()
            raise RuntimeError("DQN update produced a non-finite gradient norm")
        self.optimizer.step()
...
class GreedyPolicy:
    ...
    def act(self, observation: np.ndarray, policy_mode: str = "greedy") -> int:
        if policy_mode != "greedy":
            raise ValueError("DQN evaluates greedily only")
        observation = np.asarray(observation, dtype=np.float32)
        if observation.shape != (self.observation_dim,):
