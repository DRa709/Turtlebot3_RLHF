"""ROS 2 (Foxy) Discrete SAC agent node. Algorithm-specific file.

All scheduling lives in the shared :mod:`orchestrator`; the learner is
:class:`discretesac.DiscreteSACAgent`. This node converts messages, executes effects, records
...
EXIT_FATAL = 3
EXIT_INTERRUPTED = 4
AGENT_CONFIG_NAME = "phase1_discretesac.yaml"
NODE_NAME = "discretesac_agent"


class DiscreteSACAgentNode(Node):
    def __init__(self) -> None:
        ...
        if str(self.identity["algorithm_version"]) != str(params["algorithm_version"]):
            raise ValueError("algorithm_version differs between run identity and Discrete SAC configuration")
        ...
        learner_config = DiscreteSACConfig(
            gamma=float(params["gamma"]), actor_learning_rate=float(params["actor_learning_rate"]),
            critic_learning_rate=float(params["critic_learning_rate"]), alpha=float(params["alpha"]),
            ...
            gradient_clip_norm=float(params["gradient_clip_norm"]), torch_threads=int(params["torch_threads"]),
        )
        self.learner = DiscreteSACAgent(OBSERVATION_DIM, len(commands), learner_config, self.device, learning_seed)
        ...
        orch_config = orc.OrchestratorConfig(
            ...
            eval_checkpoint_step=eval_step, eval_checkpoint_index=eval_index,
            evaluation_seed=int(self.identity["evaluation_seed"]),
            policy_modes=("stochastic", "deterministic"),
        )
        self.orchestrator = orc.Orchestrator(
            orch_config, self.learner, lambda path: DiscreteSACPolicy(path, self.device), scenarios,
    ...
    def _housekeeping(self) -> None:
        if self.interrupt_requested and self.finished_reason is None:
            self.interrupt_requested = False
            path = os.path.join(self.run_dir, "checkpoints", f"discretesac_seed{self.learner.seed}_step{self.learner.env_steps}_interrupted_full.pt")
            try:
...
AGENT_NODE_CLASS = DiscreteSACAgentNode


def main(args=None) -> None:
    rclpy.init(args=args)
    node: Optional[DiscreteSACAgentNode] = None
    exit_code = 0
    try:
        node = DiscreteSACAgentNode()
        rclpy.spin(node)
