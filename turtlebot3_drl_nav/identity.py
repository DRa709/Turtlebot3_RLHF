...
FORBIDDEN_RELEASE_NAMES = {"__pycache__", ".git", ".pytest_cache", "build", "install", "log"}
FORBIDDEN_RELEASE_SUFFIXES = (".pyc", ".pyo", ".egg-info", ".ipynb")
...
def release_files(package_root: str) -> List[str]:
    ...
    for base, dirs, files in os.walk(package_root):
        dirs[:] = sorted(
            d for d in dirs
            if d not in FORBIDDEN_RELEASE_NAMES and not d.endswith(".egg-info")
        )
        for name in sorted(files):
            if name == RELEASE_MANIFEST or name.endswith(FORBIDDEN_RELEASE_SUFFIXES):
                continue
            out.append(os.path.relpath(os.path.join(base, name), package_root).replace("\\", "/"))
    return sorted(out)


def forbidden_release_paths(package_root: str) -> List[str]:
    """Return executable/cache/build paths that may never enter a release.

    These paths are rejected, not merely omitted from the manifest. In
    particular, checked-hash bytecode can override authenticated source and
    therefore cannot be permitted anywhere in a package tree.
    """
    found: List[str] = []
    for base, dirs, files in os.walk(package_root, followlinks=False):
        retained: List[str] = []
        for name in sorted(dirs):
            rel = os.path.relpath(os.path.join(base, name), package_root)
            if name in FORBIDDEN_RELEASE_NAMES or name.endswith(".egg-info"):
                found.append(rel + "/")
            else:
                retained.append(name)
        dirs[:] = retained
        for name in sorted(files):
            if name in FORBIDDEN_RELEASE_NAMES or name.endswith(FORBIDDEN_RELEASE_SUFFIXES):
                found.append(os.path.relpath(os.path.join(base, name), package_root))
    return sorted(found)
...
def verify_manifest(package_root: str, manifest_name: str) -> List[str]:
    ...
    if manifest_name == RELEASE_MANIFEST:
        problems.extend(f"forbidden release path {rel}" for rel in forbidden_release_paths(package_root))
    declared: List[str] = []
    with open(path, encoding="utf-8") as stream:
        for line in stream:
            line = line.rstrip("\n")
