"""ROS 2 (Foxy) SD-SAC agent node. Algorithm-specific file.

All scheduling lives in the shared :mod:`orchestrator`; the learner is
:class:`sdsac.SDSACAgent`. This node converts messages, executes effects, records
...
EXIT_FATAL = 3
EXIT_INTERRUPTED = 4
AGENT_CONFIG_NAME = "phase1_sdsac.yaml"
NODE_NAME = "sdsac_agent"


class SDSACAgentNode(Node):
    def __init__(self) -> None:
        ...
        if str(self.identity["algorithm_version"]) != str(params["algorithm_version"]):
            raise ValueError("algorithm_version differs between run identity and SD-SAC configuration")
        ...
        learner_config = SDSACConfig(
            ...
            entropy_penalty_beta=float(params["entropy_penalty_beta"]),
            q_clip_range=float(params["q_clip_range"]),
        ...
        self.learner = SDSACAgent(OBSERVATION_DIM, len(commands), learner_config, self.device, learning_seed)
        ...
        self.orchestrator = orc.Orchestrator(
            orch_config, self.learner, lambda path: SDSACPolicy(path, self.device), scenarios,
    ...
    def _housekeeping(self) -> None:
        if self.interrupt_requested and self.finished_reason is None:
            self.interrupt_requested = False
            path = os.path.join(self.run_dir, "checkpoints", f"sdsac_seed{self.learner.seed}_step{self.learner.env_steps}_interrupted_full.pt")
            try:
...
AGENT_NODE_CLASS = SDSACAgentNode


def main(args=None) -> None:
    rclpy.init(args=args)
    node: Optional[SDSACAgentNode] = None
    exit_code = 0
    try:
        node = SDSACAgentNode()
        rclpy.spin(node)
