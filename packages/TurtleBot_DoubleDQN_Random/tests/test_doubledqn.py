"""Double DQN learner tests (roadmap §3.2): independent recomputation of the target,
...
try:
    ...
    raise unittest.SkipTest("PyTorch is required for the DoubleDQN learner tests") from exc
from torch import nn

from turtlebot3_drl_nav.doubledqn import ALGORITHM, DoubleDQNAgent, DoubleDQNConfig, GreedyPolicy, parameter_digest
...
class DoubleDQNUpdateTests(unittest.TestCase):
    def test_update_matches_independent_recomputation_bit_exactly(self):
        cfg = DoubleDQNConfig(gamma=0.99, batch_size=8, warmup_steps=8, replay_capacity=64, target_update_steps=3, hidden_size=16, loss="huber")
        agent = DoubleDQNAgent(OBS, ACT, cfg, torch.device("cpu"), seed=7)
    ...
    def test_truncation_bootstraps_and_termination_does_not(self):
        cfg = DoubleDQNConfig(gamma=0.5, batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=2, target_update_steps=100, loss="mse")
        agent = DoubleDQNAgent(2, 2, cfg, torch.device("cpu"), seed=1)
        with torch.no_grad():
    ...
    def test_online_selects_and_target_evaluates_next_action(self):
        cfg = DoubleDQNConfig(gamma=0.5, batch_size=2, warmup_steps=2, hidden_size=4,
                              replay_capacity=2, target_update_steps=100, loss="mse")
        agent = DoubleDQNAgent(2, 2, cfg, torch.device("cpu"), seed=11)
        with torch.no_grad():
            ...
            agent.online.net[-1].bias.copy_(torch.tensor([0.0, 5.0]))
            agent.target.net[-1].bias.copy_(torch.tensor([7.0, 1.0]))
    ...
    def test_warmup_target_sync_and_epsilon_schedule(self):
        cfg = DoubleDQNConfig(batch_size=2, warmup_steps=5, hidden_size=4, replay_capacity=16, target_update_steps=3, epsilon_decay_steps=20)
        agent = DoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=2)
    ...
    def test_evaluation_is_greedy_and_deterministic(self):
        cfg = DoubleDQNConfig(batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=8)
        agent = DoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=3)
    ...
    def test_atomic_checkpoint_and_greedy_policy_round_trip(self):
        cfg = DoubleDQNConfig(batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=8)
        agent = DoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=4)
        o = np.zeros(4, dtype=np.float32)
        for _ in range(3):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            restored = DoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=99)
    ...
    def test_config_validation(self):
        with self.assertRaises(ValueError):
            DoubleDQNConfig(warmup_steps=10, batch_size=64).validate()
        with self.assertRaises(ValueError):
            DoubleDQNConfig(epsilon_start=0.1, epsilon_end=0.5).validate()
        with self.assertRaises(ValueError):
            DoubleDQNConfig(loss="l1").validate()
        with self.assertRaises(ValueError):
            DoubleDQNConfig(learning_rate=0.0).validate()
        with self.assertRaises(ValueError):
            DoubleDQNConfig(batch_size=64.0).validate()

    def test_nonfinite_or_out_of_range_transition_is_refused(self):
        agent = DoubleDQNAgent(4, 3, DoubleDQNConfig(batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=8), torch.device("cpu"), seed=5)
    ...
    def test_replay_ring_overwrites_without_copying_the_whole_buffer(self):
        rng = random.Random(1)
        from turtlebot3_drl_nav.doubledqn import ReplayBuffer
