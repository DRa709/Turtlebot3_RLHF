...
class LoopHarness:
    def __init__(self, run_dir: str, seed: int = 101, budget: int = 60, checkpoint_interval: int = 20,
        ...
        self.learner = make_learner(OBSERVATION_DIM, 5, seed, torch.device("cpu"), batch_size=4, replay_capacity=64,
                                    warmup_steps=8, target_update_steps=5, epsilon_decay_steps=40, hidden_size=8, torch_threads=1)
        ocfg = orc.OrchestratorConfig(
            ...
            eval_checkpoint_index=budget // checkpoint_interval,
        )
    ...
    def apply_engine_effects(self, effects) -> None:
        for effect in effects:
            if isinstance(effect, ee.PublishState):
                ...
                # The acknowledgement may emit the step-0 transition.  Keep
                # that effect on the harness path so it reaches transitions.csv
                # rather than FakeSim's private in-memory transition list.
