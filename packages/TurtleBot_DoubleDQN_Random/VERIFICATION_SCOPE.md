...
Discrete SAC and SD-SAC. PPO, A2C and other on-policy methods are future work
and are not execution targets in this study.
...
7. **ARC:** Foxy/Gazebo/Apptainer consistency, exclusive Slurm allocation,
   leased ROS/Gazebo identities, explicit `--cleanenv` forwarding, live-process
   monitoring, walltime handling and non-reused run directories.
