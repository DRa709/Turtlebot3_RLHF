# Data contract — canonical run streams (Double DQN; scientific core 3.0.2)
...
Training rows use `policy_mode=epsilon_greedy`; every Double DQN evaluation row
...
One row per Double DQN gradient step from the warm-up threshold through the
budget:
...
by the online-network argmax. The schema reserves explicit empty fields for
mechanisms used by later standalone packages. `UPDATE_APPLICABILITY` enforces
both directions: Double-DQN-required fields must be finite and inapplicable
