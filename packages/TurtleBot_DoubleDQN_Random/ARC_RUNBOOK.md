# ARC runbook — TurtleBot_DoubleDQN_Random 1.0.1
...
/projects/<allocation>/tb3/
  packages/TurtleBot_DoubleDQN_Random/
  images/TurtleBot_DoubleDQN_Random_v1.0.1.sif
  results/
```
...
cd /projects/<allocation>/tb3/packages/TurtleBot_DoubleDQN_Random
apptainer build /projects/<allocation>/tb3/images/TurtleBot_DoubleDQN_Random_v1.0.1.sif \
  apptainer/tb3_phase1_foxy.def
sha256sum /projects/<allocation>/tb3/images/TurtleBot_DoubleDQN_Random_v1.0.1.sif \
  > /projects/<allocation>/tb3/images/TurtleBot_DoubleDQN_Random_v1.0.1.sif.sha256
apptainer test /projects/<allocation>/tb3/images/TurtleBot_DoubleDQN_Random_v1.0.1.sif
```

Preserve the exact SIF used for the campaign. Every task recomputes its bytes'
SHA-256; a sidecar is informative, not authoritative.
The image contains the runtime for this standalone Double DQN package. Preserve
its exact bytes and use its recorded digest for every Double DQN training and
...
It verifies the exact package inventory and shared layer, refuses notebooks,
links, CRLF and bad shell/Python syntax, and reproduces the E2 scenario list.
...
apptainer exec --cleanenv \
  ...
  /projects/<allocation>/tb3/images/TurtleBot_DoubleDQN_Random_v1.0.1.sif \
  bash -c 'cd /projects/<allocation>/tb3/packages/TurtleBot_DoubleDQN_Random && bash scripts/run_tests.sh'
```
...
```bash
export TB3_REPO=/projects/<allocation>/tb3/packages/TurtleBot_DoubleDQN_Random
export TB3_IMAGE=/projects/<allocation>/tb3/images/TurtleBot_DoubleDQN_Random_v1.0.1.sif
...
sbatch --account=<acct> --partition=<part> \
  --export=ALL,PHASE_LABEL=calibration --array=0 arc/doubledqn_array.sbatch
```
...
sbatch --account=<acct> --partition=<part> \
  ...
  --array=0 arc/doubledqn_eval_array.sbatch
```
...
```bash
sbatch --account=<acct> --partition=<part> \
  --export=ALL,PHASE_LABEL=pilot --array=0-1 arc/doubledqn_array.sbatch
sbatch --account=<acct> --partition=<part> \
  --export=ALL,PHASE_LABEL=pilot,TRAIN_JOB=<training-array-id> \
  --array=0-1 arc/doubledqn_eval_array.sbatch
```
...
```bash
sbatch --account=<acct> --partition=<part> \
  --export=ALL,PHASE_LABEL=controlled --array=0-4 arc/doubledqn_array.sbatch
```
...
```bash
tail -f "$TB3_OUTPUT/controlled/doubledqn/seed_101/job_<id>/logs/training_console.log"
python3 -m json.tool "$TB3_OUTPUT/controlled/doubledqn/seed_101/job_<id>/progress.json"
```
...
for STEP in 100000 200000 300000 400000 500000; do
  sbatch --account=<acct> --partition=<part> \
    ...
    --array=0-4 arc/doubledqn_eval_array.sbatch
done
```
...
Run on a compute node inside the preserved Double DQN SIF. Point `--results`
only at the standalone Double DQN controlled-result root.
...
apptainer exec --cleanenv \
  ...
  "$TB3_IMAGE" bash -c '
    ...
    python3 scripts/make_tables.py \
      --results '"$TB3_OUTPUT"'/controlled/doubledqn \
      --out '"$TB3_OUTPUT"'/analysis/doubledqn/tables \
      --phase-label controlled \
      --expected-algorithms DoubleDQN &&
    python3 scripts/make_figures.py \
      --results '"$TB3_OUTPUT"'/controlled/doubledqn \
      --out '"$TB3_OUTPUT"'/analysis/doubledqn/figures \
...
With `--expected-algorithms DoubleDQN`, analysis fails unless Double DQN has
exactly one training run for every preregistered seed and one evaluation run for
every preregistered seed/checkpoint. It also requires common randomization
