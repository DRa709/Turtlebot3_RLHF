#!/usr/bin/env python3
"""Draw Discrete SAC random-arm figures from completed run directories and
...
def main() -> int:
    ...
    if not args.include_incomplete:
        analysis.require_standalone_complete(
            runs, load_evaluation_protocol(os.path.join(ROOT, "config")), args.phase_label,
        )
