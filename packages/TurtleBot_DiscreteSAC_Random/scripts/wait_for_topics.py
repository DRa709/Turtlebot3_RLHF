...
ROS 2 Foxy's ``ros2 topic echo`` command has no portable one-message option.
This helper subscribes directly, accepts best-effort publishers, requires one
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        # Keep subscription objects live through the final spin.
        subscriptions.clear()
        node.destroy_node()
        rclpy.shutdown()
