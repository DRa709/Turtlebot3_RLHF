...
def evaluation_run(root, name, step, seed=101):
    ...
    with open(os.path.join(run_dir, "evaluation.csv"), "w", encoding="utf-8") as stream:
        ...
        stream.write(f"{step}\n")
    return Run(run_dir, {
        "algorithm": "DiscreteSAC", "learning_seed": seed, "phase_label": "controlled", "phase_type": "evaluation",
    })
...
def standalone_run(root, name, phase_type, step=None, seed=101):
    ...
    os.makedirs(run_dir)
    identity = {
        "algorithm": "DiscreteSAC", "learning_seed": seed, "phase_label": "controlled",
        "phase_type": phase_type, "run_id": f"DiscreteSAC-{phase_type}-{seed}",
    ...
    if step is not None:
        ...
        with open(os.path.join(run_dir, "run_manifest.json"), "w", encoding="utf-8") as stream:
            json.dump({"training_run_id": "DiscreteSAC-training-101"}, stream)
...
class AnalysisCoreTests(unittest.TestCase):
    ...
    def test_standalone_completeness_rejects_missing_and_extra_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(report["passed"])
            self.assertTrue(any("training run for DiscreteSAC seed 202" in error for error in report["errors"]))
