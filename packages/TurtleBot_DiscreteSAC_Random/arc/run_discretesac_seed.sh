#!/usr/bin/env bash
# ARC training wrapper for one (algorithm=DiscreteSAC, learning seed) run. Runs inside
...
# a non-zero status.
#
# Required environment (set by arc/discretesac_array.sbatch):
...
# Optional: TB3_WS (workspace path), JOB_TOKEN
...
if [[ "$REPO" =~ [[:space:]] || "$RUN_DIR" =~ [[:space:]] ]]; then
  echo "repository and run paths must not contain whitespace under ROS 2 Foxy" >&2
  exit 2
fi
JOB_TOKEN="${JOB_TOKEN:-${SLURM_JOB_ID:-local}}"
WS="${TB3_WS:-$RUN_DIR/../ws}"
mkdir -p "$RUN_DIR"
...
on_walltime_warning() {
  ...
  pkill -USR1 -f "lib/turtlebot3_drl_nav/discretesac_agent" 2>/dev/null || true
...
if [[ ! -f "$WS/install/setup.bash" ]]; then
  mkdir -p "$WS/src"
  ln -sfn "$REPO" "$WS/src/turtlebot3_drl_nav"
  (cd "$WS" && colcon build --symlink-install --packages-select turtlebot3_drl_nav)
fi
# shellcheck disable=SC1090
source "$WS/install/setup.bash"
export TURTLEBOT3_MODEL=burger
export ROS_LOCALHOST_ONLY=1
export TURTLEBOT3_WS="$WS"
...
identity = build_identity(
    experiment="phase1-random-arm", algorithm="DiscreteSAC", algorithm_version="$PACKAGE_VERSION", arm="random",
