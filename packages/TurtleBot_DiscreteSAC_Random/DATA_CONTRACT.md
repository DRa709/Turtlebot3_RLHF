# Data contract — standalone Discrete SAC
...
`algorithm=DiscreteSAC`, `arm=random` runs and refuses foreign algorithms.
...
replay size, batch size, fixed alpha and learning rates, loss identities,
entropy bounds, probability bounds and non-negative gradient norms.
...
Discrete SAC matrix, reject other algorithms, and preserve policy mode as a
