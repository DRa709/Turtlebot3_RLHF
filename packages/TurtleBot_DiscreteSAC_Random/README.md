# TurtleBot3 Discrete SAC navigation 1.0.2

Discrete SAC navigation for TurtleBot3 Burger using ROS 2 Foxy, Gazebo 11, and CPU PyTorch.
This directory contains the learner, environment configuration, simulation assets,
ARC job scripts, and analysis tools for the DiscreteSAC experiment.

## Algorithm and environment

A categorical actor and twin critics compute exact expectations over five actions, with fixed entropy temperature `alpha=0.2`.

- One learner: `DiscreteSAC`.
...
Package version `1.0.2` is defined by `VERSION` and used by the package metadata
...
| Path | Purpose |
| --- | --- |
| `turtlebot3_drl_nav/discretesac.py` | Learner, updates, and checkpoints |
| `turtlebot3_drl_nav/discretesac_agent_node.py` | ROS learner interface |
| `config/phase1_discretesac.yaml` | Algorithm parameters |
