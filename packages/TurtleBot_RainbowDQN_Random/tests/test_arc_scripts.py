...
class ArcScriptContractTests(unittest.TestCase):
    def test_rainbow_identity_and_entry_points_are_standalone(self):
        train_array = source("arc/rainbowdqn_array.sbatch")
        eval_array = source("arc/rainbowdqn_eval_array.sbatch")
        train_wrapper = source("arc/run_rainbowdqn_seed.sh")
        preflight = source("scripts/preflight.sh")
        container = source("apptainer/tb3_phase1_foxy.def")
        self.assertIn('algorithm="RainbowDQN"', train_wrapper)
        self.assertIn("/rainbowdqn/seed_", train_array)
        self.assertIn("/rainbowdqn/seed_", eval_array)
        self.assertIn("run_rainbowdqn_seed.sh", train_array)
        self.assertIn("run_rainbowdqn_eval.sh", eval_array)
        self.assertIn("TurtleBot_RainbowDQN_Random", container)
        self.assertIn("Algorithm RainbowDQN", container)
        self.assertNotIn("phase1_dqn.yaml", preflight)
        self.assertNotIn("phase1_doubledqn.yaml", preflight)
        self.assertIn("must not contain whitespace under ROS 2 Foxy", source("arc/run_rainbowdqn_eval.sh"))

    def test_arrays_are_exclusive_cleanenv_and_forward_slurm_identity(self):
        for relative in ("arc/rainbowdqn_array.sbatch", "arc/rainbowdqn_eval_array.sbatch"):
    ...
    def test_wrappers_seal_outputs_before_complete(self):
        for relative in ("arc/run_rainbowdqn_seed.sh", "arc/run_rainbowdqn_eval.sh"):
            text = source(relative)
        ...
        self.assertIn("scripts/select_checkpoint.py", source("arc/run_rainbowdqn_eval.sh"))

    def test_each_slurm_task_gets_a_unique_workspace(self):
        train = source("arc/rainbowdqn_array.sbatch")
        evaluation = source("arc/rainbowdqn_eval_array.sbatch")
