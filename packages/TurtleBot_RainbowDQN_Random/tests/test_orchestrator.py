...
class TrainingLoopTests(unittest.TestCase):
    ...
    def test_updates_start_after_warmup_and_log_all_rainbow_diagnostics(self):
        ...
        for u in updates:
            self.assertEqual(u["epsilon"], "")
            for field in (
                "distributional_loss_mean", "beta_is", "mean_importance_weight",
                "max_importance_weight", "mean_priority", "max_priority",
                "noisy_sigma_mean", "noisy_sigma_min", "noisy_sigma_max",
                "n_step", "mean_effective_n_step", "value_stream_expected_mean",
                "centered_advantage_abs_mean", "chosen_distribution_entropy_mean",
                "projection_clip_low_fraction", "projection_clip_high_fraction",
                "projection_mass_error_max",
            ):
                self.assertNotEqual(u[field], "", field)
        self.assertAlmostEqual(float(updates[0]["beta_is"]), 0.4 + 8 / 500000 * 0.6, places=12)
        self.assertTrue(all(u["n_step"] == "3" for u in updates))
