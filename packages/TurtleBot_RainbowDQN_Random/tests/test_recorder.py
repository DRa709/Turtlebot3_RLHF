...
class RecorderTests(unittest.TestCase):
    ...
    def test_applicability_matrix_is_rainbow_only(self):
        for algorithm, fields in UPDATE_APPLICABILITY.items():
            self.assertTrue(set(fields) <= set(UPDATES_COLUMNS), algorithm)
        self.assertEqual(set(UPDATE_APPLICABILITY), {"RainbowDQN"})
        self.assertNotIn("epsilon", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("distributional_loss_mean", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("beta_is", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("noisy_sigma_mean", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("n_step", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("value_stream_expected_mean", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("centered_advantage_abs_mean", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertIn("projection_mass_error_max", UPDATE_APPLICABILITY["RainbowDQN"])
