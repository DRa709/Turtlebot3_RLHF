#!/usr/bin/env bash
# ARC training wrapper for one (algorithm=RainbowDQN, learning seed) run. Runs inside
...
# a non-zero status.
#
# Required environment (set by arc/rainbowdqn_array.sbatch):
...
[[ ! "$REPO" =~ [[:space:]] && ! "$RUN_DIR" =~ [[:space:]] ]] || {
  echo "package and run paths must not contain whitespace under ROS 2 Foxy" >&2
  exit 2
}
...
on_walltime_warning() {
  ...
  pkill -USR1 -f "lib/turtlebot3_drl_nav/rainbowdqn_agent" 2>/dev/null || true
...
identity = build_identity(
    experiment="phase1-random-arm", algorithm="RainbowDQN", algorithm_version="$PACKAGE_VERSION", arm="random",
