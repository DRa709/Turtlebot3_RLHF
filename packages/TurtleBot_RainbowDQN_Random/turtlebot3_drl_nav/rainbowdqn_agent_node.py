"""ROS 2 (Foxy) Rainbow DQN agent node. Algorithm-specific file.

All scheduling lives in the shared :mod:`orchestrator`; the learner is
:class:`rainbowdqn.RainbowDQNAgent`. This node converts messages, executes effects, records
...
from .rainbowdqn import ALGORITHM, RainbowDQNAgent, RainbowDQNConfig, GreedyPolicy, _torch_load, parameter_digest, validate_checkpoint_payload
...
EXIT_FATAL = 3
EXIT_INTERRUPTED = 4
AGENT_CONFIG_NAME = "phase1_rainbowdqn.yaml"
NODE_NAME = "rainbowdqn_agent"


class RainbowDQNAgentNode(Node):
    def __init__(self) -> None:
        ...
        if str(self.identity["algorithm_version"]) != str(params["algorithm_version"]):
            raise ValueError("algorithm_version differs between run identity and Rainbow DQN configuration")
        ...
        learner_config = RainbowDQNConfig(
            ...
            target_update_steps=int(params["target_update_steps"]), hidden_size=int(params["hidden_size"]),
            gradient_clip_norm=float(params["gradient_clip_norm"]), atoms=int(params["atoms"]),
            v_min=float(params["v_min"]), v_max=float(params["v_max"]), n_step=int(params["n_step"]),
            per_alpha=float(params["per_alpha"]), per_beta_start=float(params["per_beta_start"]),
            per_beta_end=float(params["per_beta_end"]), per_beta_decay_steps=int(params["per_beta_decay_steps"]),
            per_epsilon=float(params["per_epsilon"]), noisy_sigma0=float(params["noisy_sigma0"]),
            dueling_aggregation=str(params["dueling_aggregation"]),
            distributional_loss=str(params["distributional_loss"]), torch_threads=int(params["torch_threads"]),
        )
        self.learner = RainbowDQNAgent(OBSERVATION_DIM, len(commands), learner_config, self.device, learning_seed)
    ...
    def _housekeeping(self) -> None:
        if self.interrupt_requested and self.finished_reason is None:
            self.interrupt_requested = False
            path = os.path.join(self.run_dir, "checkpoints", f"rainbowdqn_seed{self.learner.seed}_step{self.learner.env_steps}_interrupted_full.pt")
            try:
...
AGENT_NODE_CLASS = RainbowDQNAgentNode


def main(args=None) -> None:
    rclpy.init(args=args)
    node: Optional[RainbowDQNAgentNode] = None
    exit_code = 0
    try:
        node = RainbowDQNAgentNode()
        rclpy.spin(node)
