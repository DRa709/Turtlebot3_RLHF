...
def figure_diagnostics(training_runs: Sequence[Run], path: str) -> None:
    ...
    import matplotlib.pyplot as plt
    fields = (
        "distributional_loss_mean", "mean_priority", "beta_is",
        "mean_importance_weight", "noisy_sigma_mean",
        "chosen_distribution_entropy_mean",
    )
...
def table_learner_diagnostics(training_runs: Sequence[Run], tail: int = 1000) -> pd.DataFrame:
    ...
    for run in training_runs:
        ...
        for field in (
            "loss", "distributional_loss_mean", "td_error_abs_mean",
            "q_taken_mean", "target_mean", "next_max_q_mean", "grad_norm",
            "beta_is", "mean_importance_weight", "max_importance_weight",
            "mean_priority", "max_priority", "noisy_sigma_mean",
            "noisy_sigma_min", "noisy_sigma_max", "mean_effective_n_step",
