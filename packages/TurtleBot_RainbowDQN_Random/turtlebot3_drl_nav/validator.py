...
def _i(value: str) -> Optional[int]:
    if value == "":
        return None
    # Seeds and episode keys are decimal integers and can use the full signed
    # 63-bit range. A float round trip loses precision above 2**53.
...
def _validate_updates(report, updates, algorithm, params, budget) -> None:
    ...
    epsilon_ok = all(u["epsilon"] == "" for u in updates)
    report.check(epsilon_ok, "Rainbow uses NoisyNet exploration and records epsilon as inapplicable")
    if algorithm == "RainbowDQN":
        beta_start = float(params["per_beta_start"])
        beta_end = float(params["per_beta_end"])
        beta_decay = int(params["per_beta_decay_steps"])
        ...
        for row in updates:
            try:
                ...
                expected_beta = beta_start + min(1.0, env_step / beta_decay) * (beta_end - beta_start)
                replay_size = _i(row["replay_size"])
                invalid = (
                    row["epsilon"] != ""
                    or abs(_f(row["beta_is"]) - expected_beta) > 1e-9
                    or _i(row["n_step"]) != n_step
                    or not 1.0 <= _f(row["mean_effective_n_step"]) <= float(n_step)
                    or _i(row["batch_size"]) != batch_size
                    or not min(env_step - n_step + 1, capacity) <= replay_size <= min(env_step, capacity)
                    or abs(_f(row["learning_rate"]) - learning_rate) > 1e-15
                    or not 0.0 < _f(row["mean_importance_weight"]) <= _f(row["max_importance_weight"]) <= 1.0 + 1e-9
                    or not 0.0 < _f(row["mean_priority"]) <= _f(row["max_priority"])
                    or not 0.0 <= _f(row["projection_clip_low_fraction"]) <= 1.0
                    or not 0.0 <= _f(row["projection_clip_high_fraction"]) <= 1.0
                    or not 0.0 <= _f(row["projection_mass_error_max"]) <= 1e-5
                    or _f(row["noisy_sigma_mean"]) < 0.0
                    or _f(row["noisy_sigma_min"]) < 0.0
                    or _f(row["noisy_sigma_max"]) < _f(row["noisy_sigma_mean"])
                    or _f(row["noisy_sigma_mean"]) < _f(row["noisy_sigma_min"])
                )
        ...
        report.check(
            rainbow_ok,
            "Rainbow beta, PER weights/priorities, n-step length, support projection, NoisyNet and replay-size diagnostics satisfy the frozen contract",
        )
...
def _validate_evaluation_training(report, evaluation, checkpoints, phase, sampler, seeds, common, transitions) -> None:
    ...
    for step, rows in per_cp.items():
        if sorted(_i(r["evaluation_episode"]) for r in rows) != list(range(1, n + 1)):
            complete = False
        for r in rows:
            if r["condition"] != "E1" or r["policy_mode"] != "greedy" or r["learner_state_unchanged"] != "1" or r["checkpoint_sha256"] != policy_cps.get(step):
                clean = False
            ...
            if not _phase_row_matches(r, phases):
                repro = False
    report.check(complete, f"exactly {n} E1 episodes per checkpoint, numbered 1..{n}")
    report.check(clean, "evaluation rows are E1/greedy, learner untouched, checkpoint digest matches, initialization verified")
