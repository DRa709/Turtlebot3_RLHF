...
Statistics follow the master roadmap's protocol: the seed is the unit of
analysis, aggregates are interquartile means with bootstrap 95 % intervals over
seeds, pairwise claims use the probability of improvement (Agarwal et al. 2021).
"""
...
def probability_of_improvement(x: Sequence[float], y: Sequence[float]) -> float:
    """P(X > Y) + 0.5 P(X = Y) over all seed pairs."""
    x, y = np.asarray(x, dtype=float), np.asarray(y, dtype=float)
    if x.size == 0 or y.size == 0:
        return float("nan")
    greater = (x[:, None] > y[None, :]).mean()
    equal = (x[:, None] == y[None, :]).mean()
    return float(greater + 0.5 * equal)


def poi_ci(x: Sequence[float], y: Sequence[float], resamples: int = BOOTSTRAP_RESAMPLES, seed: int = 0) -> Tuple[float, float]:
    x, y = np.asarray(x, dtype=float), np.asarray(y, dtype=float)
    rng = np.random.default_rng(seed)
    stats = [probability_of_improvement(x[rng.integers(0, x.size, x.size)], y[rng.integers(0, y.size, y.size)]) for _ in range(resamples)]
...
def campaign_completeness(
    ...
    """Check the cross-run evidence matrix before paper analysis.

    Per-run validators cannot notice an omitted seed/checkpoint or a campaign
    that accidentally mixes runtime images.  This check closes that gap and
    binds every tier-2 evaluation run back to its one training run.
    """
    algorithms = list(expected_algorithms)
    errors: List[str] = []
    if not algorithms or len(algorithms) != len(set(algorithms)):
        errors.append("expected algorithm list must be non-empty and unique")
    known = {"DQN", "DoubleDQN", "DuelingDoubleDQN", "RainbowDQN", "DiscreteSAC"}
    if any(name not in known for name in algorithms):
        errors.append("expected algorithm list contains an unknown package identity")
    try:
        ...
        return {"passed": False, "errors": [f"malformed campaign protocol: {error}"]}
    ...
    for run in selected:
        ...
        for seed_name, expected in common_seeds.items():
            if int(run.identity.get(seed_name, -1)) != expected:
                errors.append(f"{run.run_dir}: {seed_name} differs from the common campaign seed")
    ...
    if len(shared) != 1:
        errors.append("campaign mixes shared-layer digests")
    if len(containers) != 1:
        errors.append("campaign mixes container-image digests")
    return {
        "passed": not errors,
...
def require_campaign_complete(
    ...
    report = campaign_completeness(runs, protocol, phase_label, expected_algorithms)
    if not report["passed"]:
        raise ValueError("campaign completeness failed: " + "; ".join(report["errors"]))
    return report
...
def learning_curves(training_runs: Sequence[Run], condition: str = "E1") -> pd.DataFrame:
    ...
    for run in training_runs:
        ...
        if ev.empty:
            continue
        for step, group in block(ev, condition).groupby("checkpoint_step"):
            metrics = per_seed_metrics(group)
            metrics.update({"algorithm": run.algorithm, "seed": run.seed, "checkpoint_step": int(step)})
            rows.append(metrics)
    return pd.DataFrame(rows)
...
def table_efficiency(curves: pd.DataFrame, threshold: float = SUCCESS_THRESHOLD) -> pd.DataFrame:
    ...
    for algorithm, group in curves.groupby("algorithm"):
        ...
        rows.append({"algorithm": algorithm, "seeds": len(final), f"steps_to_success_{threshold}": format_iqm(stt, 0),
...
def table_generalization(training_runs: Sequence[Run], eval_runs: Sequence[Run]) -> pd.DataFrame:
    ...
    for run in training_runs:
        ...
        m = per_seed_metrics(block(ev, "E1", final))
        if m:
            e1[(run.algorithm, run.seed)] = m["success"]
    e2: Dict[Tuple[str, int], float] = {}
    strata: Dict[Tuple[str, str, object], List[float]] = {}
    for run in eval_runs:
        ev = run.evaluation()
        if ev.empty:
            continue
        df = block(ev, "E2", policy_mode="greedy") if "greedy" in set(ev["policy_mode"]) else block(ev, "E2", policy_mode="deterministic")
        if df.empty:
            continue
        key = (run.algorithm, run.seed)
        if key in e2:
        ...
        for column in ("distance_bin", "clearance_bin", "heading_bin", "difficulty"):
            for value, sub in df.groupby(column):
                strata.setdefault((run.algorithm, column, value), []).append(float((sub["outcome"] == "goal").mean()))
    ...
    for algorithm in algorithms:
        ...
        keys = sorted(keys_e1)
        a, b = [e1[key] for key in keys], [e2[key] for key in keys]
        gap = [e1[key] - e2[key] for key in keys]
        row = {"algorithm": algorithm, "E1_success": format_iqm(a), "E2_success": format_iqm(b), "E1_minus_E2": format_iqm(gap)}
        for (alg, column, value), values in sorted(strata.items(), key=lambda kv: (kv[0][1], str(kv[0][2]))):
            if alg == algorithm:
...
def table_pairwise(eval_runs: Sequence[Run]) -> pd.DataFrame:
    """T-R4: probability of improvement on E2 success at the final checkpoint."""
    ...
    for run in eval_runs:
        ...
        if ev.empty:
            continue
        df = block(ev, "E2")
        df = df[df["policy_mode"] == sorted(df["policy_mode"].unique())[0]]
        per_alg.setdefault(run.algorithm, []).append(float((df["outcome"] == "goal").mean()))
    algorithms = sorted(per_alg)
    rows = []
    for a in algorithms:
        row = {"algorithm": a}
        for b in algorithms:
            if a == b:
                ...
                p = probability_of_improvement(per_alg[a], per_alg[b])
                lo, hi = poi_ci(per_alg[a], per_alg[b])
                row[b] = f"{p:.2f} [{lo:.2f}, {hi:.2f}]"
        rows.append(row)
    return pd.DataFrame(rows)
...
def table_per_seed(training_runs: Sequence[Run], eval_runs: Sequence[Run]) -> pd.DataFrame:
    ...
    for run in training_runs:
        ...
        final = int(ev["checkpoint_step"].max())
        m = per_seed_metrics(block(ev, "E1", final))
        rows.append({"algorithm": run.algorithm, "seed": run.seed, "condition": "E1", "checkpoint_step": final, **m})
    for run in eval_runs:
        ev = run.evaluation()
...
TABLE_CAPTIONS = {
    ...
    "T-R4_probability_of_improvement": "Pairwise probability of improvement on E2 success (row over column).",
    ...
    for ax, metric in zip(axes, metrics):
        for algorithm, group in curves.groupby("algorithm"):
            ...
            for step in steps:
                ...
                lo.append(a)
                hi.append(b)
            line, = ax.plot(steps, centre, label=algorithm, linewidth=2)
...
def figure_spatial_success(eval_runs: Sequence[Run], arena: Arena, amplitude: float, path: str) -> None:
    ...
    for run in eval_runs:
        ev = run.evaluation()
        df = block(ev, "E2") if not ev.empty else ev
        if not df.empty:
            per_alg.setdefault(run.algorithm, []).append(df)
    if not per_alg:
        return
    width = IEEE_SINGLE_COLUMN_IN if len(per_alg) == 1 else IEEE_DOUBLE_COLUMN_IN
    fig, axes = plt.subplots(1, len(per_alg), figsize=(width, width / len(per_alg) * 1.05), squeeze=False)
    for ax, (algorithm, frames) in zip(axes[0], sorted(per_alg.items())):
        ...
        ax.set_title(f"{algorithm}: held-out start success over seeds")
...
def figure_trajectories(eval_runs: Sequence[Run], arena: Arena, amplitude: float, path: str, max_episodes: int = 40) -> None:
    ...
    for ax, (algorithm, run) in zip(axes[0], sorted(per_alg.items())):
        draw_arena(ax, arena, amplitude)
        tr = run.transitions(usecols=["phase", "episode_key", "step_in_episode", "x", "y", "condition", "goal", "collision", "safety", "truncated"])
        tr = tr[(tr["phase"] == "evaluation") & (tr["condition"] == "E2")]
        ...
        ax.plot([2.0], [0.0], marker="*", color="gold", markersize=14, markeredgecolor="black")
        ax.set_title(f"{algorithm} seed {run.seed}: E2 trajectories")
...
def figure_diagnostics(training_runs: Sequence[Run], path: str) -> None:
    ...
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    fields = ("loss", "next_max_q_mean", "grad_norm", "epsilon")
    fig, axes = plt.subplots(1, len(fields), figsize=(IEEE_DOUBLE_COLUMN_IN, 1.8))
    any_data = False
    for run in training_runs:
        up = run.updates()
        if up.empty:
...
def figure_efficiency_distributions(eval_runs: Sequence[Run], path: str) -> None:
    ...
    for run in eval_runs:
        ...
        data.setdefault(run.algorithm, []).extend(ok["path_efficiency"].dropna().tolist())
        times.setdefault(run.algorithm, []).extend(ok["time_to_goal_steps"].dropna().tolist())
...
def table_failure_anatomy(eval_runs: Sequence[Run]) -> pd.DataFrame:
    ...
    for run in eval_runs:
        ...
        if ev.empty:
            continue
        df = block(ev, "E2")
        df = df[df["policy_mode"] == sorted(df["policy_mode"].unique())[0]] if not df.empty else df
        if df.empty:
            continue
...
def table_learner_diagnostics(training_runs: Sequence[Run], tail: int = 1000) -> pd.DataFrame:
    ...
    for run in training_runs:
        ...
        for field in ("loss", "td_error_abs_mean", "q_taken_mean", "next_max_q_mean", "grad_norm", "epsilon",
                      "beta_is", "mean_priority", "noisy_sigma_mean", "mean_effective_n_step", "alpha", "policy_entropy"):
...
def figure_poi_heatmap(eval_runs: Sequence[Run], path: str) -> None:
    """F-R4: probability of improvement P(row > column) on E2 success."""
    import matplotlib.pyplot as plt
    per_alg = _e2_success_by_algorithm(eval_runs)
    algorithms = sorted(per_alg)
    if len(algorithms) < 2:
        return
    matrix = np.array([[probability_of_improvement(per_alg[a], per_alg[b]) if a != b else np.nan for b in algorithms] for a in algorithms])
    fig, ax = plt.subplots(figsize=(IEEE_SINGLE_COLUMN_IN, IEEE_SINGLE_COLUMN_IN * 0.85))
    ...
    ax.set_title("P(row improves over column), E2 success")
...
def figure_stratified_success(eval_runs: Sequence[Run], path: str) -> None:
    ...
    for run in eval_runs:
        ...
        for column in strata_cols:
            for value, sub in df.groupby(column):
                data.setdefault((run.algorithm, column, value), []).append(float((sub["outcome"] == "goal").mean()))
    if not data:
        return
...
def figure_checkpointwise_heldout(eval_runs: Sequence[Run], path: str) -> None:
    ...
    for run in eval_runs:
        ...
        if df.empty:
            continue
        df = df[df["policy_mode"] == sorted(df["policy_mode"].unique())[0]]
        points.setdefault((run.algorithm, int(df["checkpoint_step"].iloc[0])), []).append(float((df["outcome"] == "goal").mean()))
