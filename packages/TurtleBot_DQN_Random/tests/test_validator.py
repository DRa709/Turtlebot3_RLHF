...
def package_with_protocol(tmp: str, harness: LoopHarness) -> str:
    ...
    # SHARED_LAYER_FILES.txt intentionally includes the shared tests.  Keep
    # them in this temporary package so make_manifests.py can authenticate the
    # same declared inventory as the release package.
    ...
    algo[executable]["ros__parameters"].update({
        "warmup_steps": 8, "target_update_steps": 5, "epsilon_decay_steps": 40, "batch_size": 4,
    })
