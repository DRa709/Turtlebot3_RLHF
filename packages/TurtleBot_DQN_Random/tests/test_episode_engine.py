...
class ResetTransactionTests(unittest.TestCase):
    ...
    def test_episode_one_is_randomized_not_spawn_pose(self):
        ...
        run_reset(engine, sim, EpisodeSpec(phase="training", policy_mode="epsilon_greedy"))
    ...
    def test_nonfinite_realized_or_sensor_pose_fails_closed(self):
        ...
        effects = engine.start_episode(EpisodeSpec(phase="training", policy_mode="epsilon_greedy"), sim.sim_time)
    ...
    def test_obstacle_ack_timeout_is_fatal(self):
        ...
        engine.start_episode(EpisodeSpec(phase="training", policy_mode="epsilon_greedy"), sim.sim_time)
        sim.wall += 1.0
        sim.apply_effects(engine, engine.tick(sim.sim_time, sim.scan(), sim.odom(), sim.obstacle_samples()))
        self.assertEqual(engine.state, EpisodeEngine.FATAL)
...
class DeterminismTests(unittest.TestCase):
    ...
    def test_different_initialization_seed_changes_starts_but_not_obstacles(self):
        ...
        run_reset(engine_a, sim_a, EpisodeSpec(phase="training", policy_mode="epsilon_greedy"))
        run_reset(engine_b, sim_b, EpisodeSpec(phase="training", policy_mode="epsilon_greedy"))
...
class TransitionTests(unittest.TestCase):
    ...
    def test_safety_stop_precedes_goal_and_collision_precedes_safety(self):
        ...
        run_reset(engine2, sim2, EpisodeSpec(phase="training", policy_mode="epsilon_greedy"))
