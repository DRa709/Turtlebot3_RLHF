...
class FakeSim:
    ...
    def apply_obstacle_control(self, payload: Dict[str, object]) -> Dict[str, object]:
        """Apply one obstacle command and return the simulator acknowledgement.

        The closed-loop harness calls this method directly and routes the
        engine effects produced by the acknowledgement through its canonical
        recorder.  Direct episode-engine tests still use ``apply_effects`` and
        retain their in-memory recorder path.
        """
