...
class IdentityTests(unittest.TestCase):
    ...
    def test_identity_round_trip_and_phase_validation(self):
        identity = build_identity("e", "DQN", "1.0.0", "random", 101, "phase1_mixed", 7000, 7001, 7002, 9001, "c" * 64, "d" * 64, "a" * 64, "b" * 64, "1.0.0", "training", "controlled", "job")
        ...
        with self.assertRaises(ValueError):
            build_identity("e", "DQN", "1.0.0", "random", 101, "w", 1, 1, 1, 1, "c" * 64, "d" * 64, "s" * 64, "r" * 64, "1", "training", "full", "job")
        with self.assertRaises(ValueError):
            build_identity("e", "DQN", "1.0.0", "random", 101, "w", 1, 1, 1, 1, "short", "d" * 64, "s" * 64, "r" * 64, "1", "training", "controlled", "job")
    ...
    def test_release_manifest_rejects_every_forbidden_cache_and_build_path(self):
        cases = (
            "__pycache__/identity.cpython-38.pyc",
            ".pytest_cache/state",
            ".git/config",
            "build/generated.txt",
            "install/generated.txt",
            "log/runtime.txt",
        ...
        for relative in cases:
            with self.subTest(relative=relative), tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertTrue(forbidden_release_paths(tmp))
                self.assertTrue(any("forbidden release path" in item for item in verify_manifest(tmp, "RELEASE_MANIFEST.sha256")))
