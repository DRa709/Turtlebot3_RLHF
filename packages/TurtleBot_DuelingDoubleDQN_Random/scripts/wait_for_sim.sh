...
while (( SECONDS < DEADLINE )); do
  ...
  if (( ok == 1 )); then
    ...
    (( remaining > 0 )) || break
    # Foxy's topic-echo CLI has no portable one-message option.  Use a bounded rclpy
    # subscriber instead, which also proves that /clock advances and that each
    # required topic produces a message rather than merely appearing in the
    # discovery graph.
