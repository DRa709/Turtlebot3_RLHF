...
The Foxy ``ros2 topic echo`` command does not implement ``--once``.  This
helper subscribes directly, accepts both reliable and best-effort publishers,
requires one message from every named topic, and requires simulation time to
advance.  It is called only after ``wait_for_sim.sh`` has discovered all
required topic and service names.
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        # Keep subscription objects live until after the final spin.
        subscriptions.clear()
        node.destroy_node()
        rclpy.shutdown()
