...
from turtlebot3_drl_nav.analysis import Run, campaign_completeness, iqm, select_final_evaluation_runs
...
def evaluation_run(root, name, step, seed=101):
    ...
    with open(os.path.join(run_dir, "evaluation.csv"), "w", encoding="utf-8") as stream:
        ...
        stream.write(f"{step}\n")
    return Run(run_dir, {
        "algorithm": "DQN", "learning_seed": seed, "phase_label": "controlled", "phase_type": "evaluation",
    })


def campaign_run(root, name, phase_type, step=None, seed=101):
    run_dir = os.path.join(root, name)
    os.makedirs(run_dir)
    identity = {
        "algorithm": "DQN", "learning_seed": seed, "phase_label": "controlled",
        "phase_type": phase_type, "run_id": f"DQN-{phase_type}-{seed}",
    ...
    if step is not None:
        ...
        with open(os.path.join(run_dir, "run_manifest.json"), "w", encoding="utf-8") as stream:
            json.dump({"training_run_id": "DQN-training-101"}, stream)
...
class AnalysisCoreTests(unittest.TestCase):
    ...
    def test_campaign_completeness_rejects_missing_and_extra_evidence(self):
        protocol = {
        ...
        with tempfile.TemporaryDirectory() as tmp:
            runs = [
                campaign_run(tmp, "train", "training"),
                campaign_run(tmp, "eval100", "evaluation", 100),
                campaign_run(tmp, "eval200", "evaluation", 200),
            ]
            self.assertTrue(campaign_completeness(runs, protocol, "controlled", ["DQN"])["passed"])
            report = campaign_completeness(runs[:-1], protocol, "controlled", ["DQN"])
            self.assertFalse(report["passed"])
            self.assertTrue(any("step 200" in error for error in report["errors"]))

            unexpected = campaign_run(tmp, "eval300", "evaluation", 300)
            report = campaign_completeness(runs + [unexpected], protocol, "controlled", ["DQN"])
            self.assertFalse(report["passed"])
            ...
            report = campaign_completeness(runs, two_seed_protocol, "controlled", ["DQN"])
            self.assertFalse(report["passed"])
            self.assertTrue(any("training run for DQN seed 202" in error for error in report["errors"]))
