...
PNG per figure, IEEE style applied. A temporary package copy carries the tiny
rendering protocol; the production campaign contract is never weakened."""
...
class AnalysisTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # The production entry points intentionally enforce the complete frozen
        # campaign.  Give this small rendering test its own temporary package
        # copy and a self-consistent one-seed/40-step pilot protocol; never
        # weaken or rewrite the production package configuration.
        cls.package_root = os.path.join(cls.tmp.name, "package")
        shutil.copytree(
            ROOT,
            cls.package_root,
            ignore=shutil.ignore_patterns("__pycache__", "*.pyc", ".pytest_cache"),
        )
        protocol_path = os.path.join(cls.package_root, "config", "common_environment.yaml")
        with open(protocol_path, encoding="utf-8") as stream:
            configuration = yaml.safe_load(stream)
        pilot = configuration["evaluation_protocol"]["phases"]["pilot"]
        pilot.update({
            "environment_budget": 40,
            "checkpoint_interval_steps": 20,
            "full_checkpoint_interval_steps": 40,
            "tier2_checkpoint_steps": [40],
            "tier1_episodes_per_checkpoint": 2,
            "learning_seeds": [101],
        })
        with open(protocol_path, "w", encoding="utf-8") as stream:
            yaml.safe_dump(configuration, stream, sort_keys=False)

        train = os.path.join(cls.tmp.name, "results", "pilot", "duelingdoubledqn", "seed_101", "job_1")
        ...
        with open(os.path.join(train, "run_manifest.json"), "w", encoding="utf-8") as stream:
            json.dump({"run_id": h.identity["run_id"], "environment_budget": 40}, stream, sort_keys=True)
            stream.write("\n")
        seal(train)
        final = [c for c in h.checkpoint_rows if c["kind"] == "policy_only"][-1]["path"]
        ev = os.path.join(cls.tmp.name, "results", "pilot", "duelingdoubledqn", "seed_101", "eval_1")
        ...
        with open(os.path.join(ev, "run_manifest.json"), "w", encoding="utf-8") as stream:
            json.dump({
                "training_run_id": h.identity["run_id"],
                "checkpoint_step": 40,
    ...
    def test_tables_in_three_formats(self):
        ...
        subprocess.run([sys.executable, os.path.join(self.package_root, "scripts", "make_tables.py"), "--results", self.results, "--out", out, "--phase-label", "pilot"], check=True, capture_output=True)
    ...
    def test_figures_as_vector_pdf_and_300dpi_png(self):
        ...
        subprocess.run([sys.executable, os.path.join(self.package_root, "scripts", "make_figures.py"), "--results", self.results, "--out", out, "--formats", "pdf,png", "--phase-label", "pilot"], check=True, capture_output=True)
