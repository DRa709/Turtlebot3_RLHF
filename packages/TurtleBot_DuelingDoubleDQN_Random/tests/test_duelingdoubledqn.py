"""Dueling Double DQN learner tests (roadmap §3.3): independent recomputation of the target,
...
and an explicit online-selection/target-evaluation discriminator."""
...
try:
    ...
    raise unittest.SkipTest("PyTorch is required for the DuelingDoubleDQN learner tests") from exc
from torch import nn

from turtlebot3_drl_nav.duelingdoubledqn import (
    ALGORITHM, DuelingDoubleDQNAgent, DuelingDoubleDQNConfig,
    DuelingQNetwork, GreedyPolicy, parameter_digest,
)
...
def reference_loss(online, target, batch, gamma, loss_name):
    ...
    with torch.no_grad():
        selected = online(s2).argmax(dim=1, keepdim=True)
        next_values = target(s2).gather(1, selected).squeeze(1)
        y = r + gamma * (1.0 - m_term) * next_values
...
class DuelingDoubleDQNUpdateTests(unittest.TestCase):
    def test_dueling_aggregation_is_mean_centered_and_identifiable(self):
        network = DuelingQNetwork(3, 4, 5)
        observation = torch.tensor([[0.2, -0.1, 0.7], [0.0, 0.5, -0.4]], dtype=torch.float32)
        with torch.no_grad():
            value, advantage = network.streams(observation)
            q_before = network(observation)
            self.assertTrue(torch.allclose(q_before.mean(dim=1, keepdim=True), value, atol=1e-7, rtol=0.0))
            network.advantage.bias.add_(3.25)
            q_after = network(observation)
        self.assertTrue(torch.allclose(q_before, q_after, atol=1e-6, rtol=0.0))
        self.assertEqual(tuple(value.shape), (2, 1))
        self.assertEqual(tuple(advantage.shape), (2, 4))

    def test_update_matches_independent_recomputation_bit_exactly(self):
        cfg = DuelingDoubleDQNConfig(gamma=0.99, batch_size=8, warmup_steps=8, replay_capacity=64, target_update_steps=3, hidden_size=16, loss="huber")
        agent = DuelingDoubleDQNAgent(OBS, ACT, cfg, torch.device("cpu"), seed=7)
    ...
    def test_truncation_bootstraps_and_termination_does_not(self):
        cfg = DuelingDoubleDQNConfig(gamma=0.5, batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=2, target_update_steps=100, loss="mse")
        agent = DuelingDoubleDQNAgent(2, 2, cfg, torch.device("cpu"), seed=1)
        with torch.no_grad():
            for p in agent.online.parameters():
                p.zero_()
            for p in agent.target.parameters():
                p.zero_()
            agent.target.value.bias.fill_(2.0)
    ...
    def test_online_selects_and_target_evaluates_next_action(self):
        cfg = DuelingDoubleDQNConfig(gamma=0.5, batch_size=2, warmup_steps=2, hidden_size=4,
                              replay_capacity=2, target_update_steps=100, loss="mse")
        agent = DuelingDoubleDQNAgent(2, 2, cfg, torch.device("cpu"), seed=11)
        with torch.no_grad():
            for parameter in agent.online.parameters():
                parameter.zero_()
            for parameter in agent.target.parameters():
                parameter.zero_()
            # Online ranks action 1 highest; target ranks action 0 highest.
            agent.online.advantage.bias.copy_(torch.tensor([0.0, 5.0]))
            agent.target.value.bias.fill_(4.0)
            agent.target.advantage.bias.copy_(torch.tensor([7.0, 1.0]))
        observation = np.zeros(2, dtype=np.float32)
        for _ in range(2):
            agent.observe(Transition(observation, 0, 0.0, observation, False))
        diagnostics = agent.optimize()
        # Double target = gamma * Q_target(s', argmax Q_online) = 0.5 * 1.0.
        self.assertAlmostEqual(diagnostics["target_mean"], 0.5, places=7)
        self.assertAlmostEqual(diagnostics["next_max_q_mean"], 1.0, places=7)
        # Standard DQN would instead use 0.5 * max Q_target = 3.5.
        self.assertNotAlmostEqual(diagnostics["target_mean"], 3.5, places=7)
        self.assertTrue(np.isfinite(diagnostics["state_value_mean"]))
        self.assertGreaterEqual(diagnostics["centered_advantage_abs_mean"], 0.0)

    def test_warmup_target_sync_and_epsilon_schedule(self):
        cfg = DuelingDoubleDQNConfig(batch_size=2, warmup_steps=5, hidden_size=4, replay_capacity=16, target_update_steps=3, epsilon_decay_steps=20)
        agent = DuelingDoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=2)
    ...
    def test_evaluation_is_greedy_and_deterministic(self):
        cfg = DuelingDoubleDQNConfig(batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=8)
        agent = DuelingDoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=3)
    ...
    def test_atomic_checkpoint_and_greedy_policy_round_trip(self):
        cfg = DuelingDoubleDQNConfig(batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=8)
        agent = DuelingDoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=4)
        o = np.zeros(4, dtype=np.float32)
        for _ in range(3):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            restored = DuelingDoubleDQNAgent(4, 3, cfg, torch.device("cpu"), seed=99)
            ...
            wrong_algorithm["algorithm"] = "DQN"
            torch.save(wrong_algorithm, policy_path)
    ...
    def test_config_validation(self):
        with self.assertRaises(ValueError):
            DuelingDoubleDQNConfig(warmup_steps=10, batch_size=64).validate()
        with self.assertRaises(ValueError):
            DuelingDoubleDQNConfig(epsilon_start=0.1, epsilon_end=0.5).validate()
        with self.assertRaises(ValueError):
            DuelingDoubleDQNConfig(loss="l1").validate()
        with self.assertRaises(ValueError):
            DuelingDoubleDQNConfig(learning_rate=0.0).validate()
        with self.assertRaises(ValueError):
            DuelingDoubleDQNConfig(batch_size=64.0).validate()
        with self.assertRaises(ValueError):
            DuelingDoubleDQNConfig(dueling_aggregation="max").validate()

    def test_nonfinite_or_out_of_range_transition_is_refused(self):
        agent = DuelingDoubleDQNAgent(4, 3, DuelingDoubleDQNConfig(batch_size=2, warmup_steps=2, hidden_size=4, replay_capacity=8), torch.device("cpu"), seed=5)
    ...
    def test_replay_ring_overwrites_without_copying_the_whole_buffer(self):
        rng = random.Random(1)
        from turtlebot3_drl_nav.duelingdoubledqn import ReplayBuffer
