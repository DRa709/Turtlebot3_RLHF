...
class RecorderTests(unittest.TestCase):
    ...
    def test_applicability_matrix_covers_every_update_column_for_every_algorithm(self):
        for algorithm, fields in UPDATE_APPLICABILITY.items():
            self.assertTrue(set(fields) <= set(UPDATES_COLUMNS), algorithm)
        self.assertEqual(set(UPDATE_APPLICABILITY), {"DQN", "DoubleDQN", "DuelingDoubleDQN", "RainbowDQN", "DiscreteSAC"})
        self.assertIn("epsilon", UPDATE_APPLICABILITY["DQN"])
        self.assertNotIn("epsilon", UPDATE_APPLICABILITY["RainbowDQN"])
        self.assertNotIn("epsilon", UPDATE_APPLICABILITY["DiscreteSAC"])
