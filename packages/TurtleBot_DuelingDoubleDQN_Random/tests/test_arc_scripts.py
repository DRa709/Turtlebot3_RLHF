...
class ArcScriptContractTests(unittest.TestCase):
    def test_dueling_double_dqn_identity_and_entry_points_are_standalone(self):
        train_array = source("arc/duelingdoubledqn_array.sbatch")
        eval_array = source("arc/duelingdoubledqn_eval_array.sbatch")
        train_wrapper = source("arc/run_duelingdoubledqn_seed.sh")
        preflight = source("scripts/preflight.sh")
        container = source("apptainer/tb3_phase1_foxy.def")
        runbook = source("ARC_RUNBOOK.md")
        self.assertIn('algorithm="DuelingDoubleDQN"', train_wrapper)
        self.assertNotIn('algorithm="DQN"', train_wrapper)
        self.assertIn("/duelingdoubledqn/seed_", train_array)
        self.assertIn("/duelingdoubledqn/seed_", eval_array)
        self.assertIn("run_duelingdoubledqn_seed.sh", train_array)
        self.assertIn("run_duelingdoubledqn_eval.sh", eval_array)
        self.assertIn("TurtleBot_DuelingDoubleDQN_Random", container)
        self.assertIn("Algorithm DuelingDoubleDQN", container)
        self.assertNotIn("TurtleBot_DQN_Random", container)
        self.assertNotIn("TurtleBot_DoubleDQN_Random", container)
        self.assertNotIn("phase1_dqn.yaml", preflight)
        self.assertNotIn("phase1_doubledqn.yaml", preflight)
        self.assertIn("--expected-algorithms DuelingDoubleDQN", runbook)
        self.assertNotIn("--expected-algorithms DQN,", runbook)
        self.assertIn("must not contain whitespace under ROS 2 Foxy", source("arc/run_duelingdoubledqn_eval.sh"))

    def test_arrays_are_exclusive_cleanenv_and_forward_slurm_identity(self):
        for relative in ("arc/duelingdoubledqn_array.sbatch", "arc/duelingdoubledqn_eval_array.sbatch"):
    ...
    def test_each_slurm_task_gets_a_unique_workspace(self):
        train = source("arc/duelingdoubledqn_array.sbatch")
        evaluation = source("arc/duelingdoubledqn_eval_array.sbatch")
    ...
    def test_wrappers_seal_outputs_before_complete(self):
        for relative in ("arc/run_duelingdoubledqn_seed.sh", "arc/run_duelingdoubledqn_eval.sh"):
            text = source(relative)
        ...
        self.assertIn("scripts/select_checkpoint.py", source("arc/run_duelingdoubledqn_eval.sh"))
