"""ROS 2 (Foxy) Dueling Double DQN agent node. Algorithm-specific file.

All scheduling lives in the shared :mod:`orchestrator`; the learner is
:class:`duelingdoubledqn.DuelingDoubleDQNAgent`. This node converts messages, executes effects, records
...
from .duelingdoubledqn import ALGORITHM, DuelingDoubleDQNAgent, DuelingDoubleDQNConfig, GreedyPolicy, _torch_load, parameter_digest, validate_checkpoint_payload
...
EXIT_FATAL = 3
EXIT_INTERRUPTED = 4
AGENT_CONFIG_NAME = "phase1_duelingdoubledqn.yaml"
NODE_NAME = "duelingdoubledqn_agent"


class DuelingDoubleDQNAgentNode(Node):
    def __init__(self) -> None:
        ...
        if str(self.identity["algorithm_version"]) != str(params["algorithm_version"]):
            raise ValueError("algorithm_version differs between run identity and Dueling Double DQN configuration")
        ...
        learner_config = DuelingDoubleDQNConfig(
        ...
        self.learner = DuelingDoubleDQNAgent(OBSERVATION_DIM, len(commands), learner_config, self.device, learning_seed)
    ...
    def _housekeeping(self) -> None:
        if self.interrupt_requested and self.finished_reason is None:
            self.interrupt_requested = False
            path = os.path.join(self.run_dir, "checkpoints", f"duelingdoubledqn_seed{self.learner.seed}_step{self.learner.env_steps}_interrupted_full.pt")
            try:
...
AGENT_NODE_CLASS = DuelingDoubleDQNAgentNode


def main(args=None) -> None:
    rclpy.init(args=args)
    node: Optional[DuelingDoubleDQNAgentNode] = None
    exit_code = 0
    try:
        node = DuelingDoubleDQNAgentNode()
        rclpy.spin(node)
