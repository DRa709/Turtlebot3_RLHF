"""Phase-1 Dueling Double DQN learner (roadmap §3.3). Algorithm-specific; no ROS dependency.

Dueling aggregation:
    Q_theta(o, a) = V_theta(o) + A_theta(o, a)
                      - mean_b A_theta(o, b)

Update rule, with a shipped test that independently recomputes the target,
loss and Adam step:
...
class DuelingDoubleDQNConfig:
    ...
    def validate(self) -> None:
        ...
        if not all(np.isfinite(float(value)) for value in scalar_values):
            raise ValueError("Dueling Double DQN scalar hyperparameters must be finite")
        integer_values = (
        ...
        if any(isinstance(value, bool) or not isinstance(value, int) for value in integer_values):
            raise ValueError("Dueling Double DQN step counts and sizes must be integers")
...
class DuelingQNetwork(nn.Module):
    """Shared feature trunk with separately identifiable value and advantage heads.

    Mean-centering the advantage stream fixes the additive identifiability
    ambiguity between V and A while leaving one Q value per discrete action.
    """
    ...
    def streams(self, observation: torch.Tensor):
        features = self.trunk(observation)
        return self.value(features), self.advantage(features)
...
def validate_checkpoint_payload(payload: Dict, allowed_kinds: tuple = ("full", "policy_only")) -> DuelingDoubleDQNConfig:
    ...
    try:
        ...
        config = DuelingDoubleDQNConfig(**payload["config"])
...
class DuelingDoubleDQNAgent:
    def __init__(self, observation_dim: int, action_dim: int, config: DuelingDoubleDQNConfig, device: torch.device, seed: int) -> None:
        config.validate()
        ...
        self.online = DuelingQNetwork(observation_dim, action_dim, config.hidden_size).to(device)
        self.target = DuelingQNetwork(observation_dim, action_dim, config.hidden_size).to(device)
    ...
    def optimize(self) -> Optional[Dict[str, float]]:
        ...
        state_values, raw_advantages = self.online.streams(states)
        centered_advantages = raw_advantages - raw_advantages.mean(dim=1, keepdim=True)
        online_q = state_values + centered_advantages
        predictions = online_q.gather(1, actions).squeeze(1)
        with torch.no_grad():
            # Dueling Double DQN decouples the maximization: the online network selects
        ...
        if not torch.isfinite(predictions).all() or not torch.isfinite(targets).all() or not torch.isfinite(loss):
            raise RuntimeError("Dueling Double DQN update produced a non-finite prediction, target, or loss")
        self.optimizer.zero_grad()
        loss.backward()
        grad_norm = nn.utils.clip_grad_norm_(self.online.parameters(), self.config.gradient_clip_norm)
        if not torch.isfinite(grad_norm):
            self.optimizer.zero_grad()
            raise RuntimeError("Dueling Double DQN update produced a non-finite gradient norm")
        self.optimizer.step()
        ...
        return {
            ...
            "state_value_mean": float(state_values.detach().mean().item()),
            "centered_advantage_abs_mean": float(centered_advantages.detach().abs().mean().item()),
        }
...
class GreedyPolicy:
    ...
    def __init__(self, path: str, device: torch.device) -> None:
        ...
        self.env_steps = int(payload["env_steps"])
        self.network = DuelingQNetwork(self.observation_dim, self.action_dim, config.hidden_size).to(device)
    ...
    def act(self, observation: np.ndarray, policy_mode: str = "greedy") -> int:
        if policy_mode != "greedy":
            raise ValueError("Dueling Double DQN evaluates greedily only")
        observation = np.asarray(observation, dtype=np.float32)
        if observation.shape != (self.observation_dim,):
