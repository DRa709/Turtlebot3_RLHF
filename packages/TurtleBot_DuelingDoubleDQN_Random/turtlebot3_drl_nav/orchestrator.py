...
Learner interface (duck-typed):
    ...
    observe(Transition); optimize() -> Optional[dict]; save(path, kind); env_steps; gradient_steps
...
returning an object with ``act(observation) -> int`` and ``parameter_digest``.
"""
...
from .initialization import Scenario
...
class OrchestratorConfig:
    ...
    policy_modes: Sequence[str] = ("greedy",)
...
class Orchestrator:
    ...
    def _start_training_episode(self) -> List[object]:
        mode = (
            "noisy" if self.cfg.algorithm == "RainbowDQN" else
            "stochastic" if self.cfg.algorithm == "DiscreteSAC" else
            "epsilon_greedy"
        )
    ...
    def _decide(self, sequence: int) -> List[object]:
        ...
        if spec.phase == "training":
            ...
            choice = None
            action = int(self.active_policy.act(observation, spec.policy_mode))
    ...
    def on_step(self, msg: StepMessage) -> List[object]:
        ...
        spec = self.current_spec
        if spec.phase == "training":
            self.learner.observe(Transition(self.current_observation.copy(), self.pending_action, msg.reward_total, msg.observation.copy(), msg.terminated))
            ...
            if diagnostics is not None:
                ...
                row.update({
                    ...
                    "epsilon": self.pending_choice.epsilon_used,
                })
    ...
    def _checkpoint_if_due(self) -> List[object]:
        ...
        episodes = [
            EpisodeSpec(phase="evaluation", policy_mode="greedy", condition="E1", checkpoint_step=step,
