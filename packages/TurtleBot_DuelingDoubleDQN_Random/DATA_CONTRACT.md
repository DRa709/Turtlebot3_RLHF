# Data contract — canonical run streams (Dueling Double DQN; scientific core 3.0.2)
...
Training rows use `policy_mode=epsilon_greedy`; every Dueling Double DQN evaluation row
uses `policy_mode=greedy`. This prevents exploratory training behavior from
being mislabeled as deployment behavior.
...
One row per Dueling Double DQN gradient step from the warm-up threshold through the
budget:
...
batch_terminal_fraction, batch_size, replay_size, state_value_mean,
centered_advantage_abs_mean`.

Here `next_max_q_mean` denotes the target-network value at the action selected
by the online-network argmax. `state_value_mean` is the batch mean of the scalar
value head. `centered_advantage_abs_mean` is the mean absolute centered
advantage over the batch and all five actions. The schema reserves explicit
empty fields for mechanisms used by other standalone packages.
`UPDATE_APPLICABILITY` enforces both directions: Dueling-Double-DQN-required
fields must be finite and inapplicable fields must be blank.
...
Dueling Double DQN uses only `policy_mode=greedy`. E1 occurs in the
...
select exactly one largest-checkpoint evaluation per Dueling Double DQN seed; duplicate
finals are an error. F-R12 alone uses all configured tier-2 checkpoints.
Supplying `--expected-algorithms DuelingDoubleDQN` activates the Dueling-Double-DQN-only stop
...
linked to their training run are fatal. A passing
`campaign_completeness.json` is written with the table data.
