...
#
# Required environment (set by arc/duelingdoubledqn_eval_array.sbatch):
...
[[ ! "$REPO" =~ [[:space:]] && ! "$TRAIN_RUN_DIR" =~ [[:space:]] && ! "$RUN_DIR" =~ [[:space:]] ]] || {
  echo "package, training-run and evaluation-run paths must not contain whitespace under ROS 2 Foxy" >&2
  exit 2
}
