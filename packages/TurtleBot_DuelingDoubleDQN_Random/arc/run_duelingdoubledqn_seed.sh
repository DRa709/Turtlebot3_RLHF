#!/usr/bin/env bash
# ARC training wrapper for one (algorithm=DuelingDoubleDQN, learning seed) run. Runs inside
...
# a non-zero status.
#
# Required environment (set by arc/duelingdoubledqn_array.sbatch):
...
on_walltime_warning() {
  ...
  pkill -USR1 -f "lib/turtlebot3_drl_nav/duelingdoubledqn_agent" 2>/dev/null || true
...
identity = build_identity(
    experiment="phase1-random-arm", algorithm="DuelingDoubleDQN", algorithm_version="$PACKAGE_VERSION", arm="random",
