"""Executable mathematical, interaction and checkpoint tests for SD-SAC."""
...
try:
    ...
    raise unittest.SkipTest("PyTorch is required for the SD-SAC learner tests") from exc

from turtlebot3_drl_nav.sdsac import (
    ALGORITHM,
    CategoricalActor,
    SDSACAgent,
    SDSACConfig,
...
def small_config(**overrides):
    ...
    values.update(entropy_penalty_beta=0.5, q_clip_range=0.5)
    values.update(overrides)
    return SDSACConfig(**values)


class SDSACMathTests(unittest.TestCase):
    ...
    def test_soft_value_uses_exact_sum_entropy_and_average_critic(self):
        ...
        expected = torch.stack([
            0.25 * (2.5 - 0.2 * logs[0, 0]) + 0.75 * (2.5 - 0.2 * logs[0, 1]),
            0.60 * (1.5 - 0.2 * logs[1, 0]) + 0.40 * (6.5 - 0.2 * logs[1, 1]),
        ])
    ...
    def test_actor_objective_uses_average_q_and_entropy_change_penalty(self):
        p = torch.tensor([[0.2, 0.3, 0.5]], requires_grad=True)
        logp = p.log()
        q1 = torch.tensor([[1.0, 4.0, -1.0]])
        q2 = torch.tensor([[2.0, 3.0, 6.0]])
        old_entropy = torch.tensor([1.2])
        actual = actor_objective(p, logp, q1, q2, old_entropy, 0.2, 0.5)
        avg = torch.tensor([[1.5, 3.5, 2.5]])
        entropy = -(p * logp).sum(dim=1)
        expected = (p * (0.2 * logp - avg)).sum() + 0.25 * (old_entropy - entropy).square().mean()
        self.assertTrue(torch.equal(actual, expected))
        actual.backward()
        self.assertIsNotNone(p.grad)
        self.assertTrue(all(value.grad is None for value in (q1, q2)))

    def test_q_clip_loss_is_elementwise_maximum(self):
        online = torch.tensor([0.2, 2.0, -2.0], requires_grad=True)
        target_current = torch.zeros(3)
        target = torch.tensor([1.0, 1.0, 1.0])
        clipped = clipped_q_value(online, target_current, 0.5)
        self.assertTrue(torch.equal(clipped, torch.tensor([0.2, 0.5, -0.5])))
        expected = torch.maximum((online - target).square(), (clipped - target).square()).mean()
        actual = q_clip_loss(online, target_current, target, 0.5)
        self.assertTrue(torch.equal(actual, expected))
        actual.backward()
        self.assertIsNotNone(online.grad)

    def test_entropy_helpers_match_definition(self):
        p = torch.tensor([[0.25, 0.75]])
        current = policy_entropy(p, p.log())
        old = torch.tensor([0.8])
        self.assertTrue(torch.equal(average_q(torch.tensor([1.0]), torch.tensor([3.0])), torch.tensor([2.0])))
        self.assertTrue(torch.equal(entropy_change_penalty(old, current, 0.5), 0.25 * (old - current).square().mean()))
...
class SDSACAgentTests(unittest.TestCase):
    def test_warmup_is_uniform_random_and_has_no_gradient_step(self):
        agent = SDSACAgent(4, 3, small_config(warmup_steps=4, batch_size=2),
                                 torch.device("cpu"), 11)
        actions = []
        for step in range(4):
    ...
    def test_replay_records_uniform_warmup_then_actor_behavior_entropy(self):
        agent = SDSACAgent(4, 3, small_config(warmup_steps=2, batch_size=2),
                           torch.device("cpu"), 110)
        agent.observe(transition(action=0, value=0.0))
        self.assertAlmostEqual(agent.replay.buffer[0].old_entropy, math.log(3.0), places=7)
        agent.observe(transition(action=1, value=0.01))
        with torch.no_grad():
            for parameter in agent.actor.parameters():
                parameter.zero_()
            agent.actor.network[-1].bias.copy_(torch.tensor([3.0, 0.0, -2.0]))
            state = torch.as_tensor(obs(4, 0.2)).unsqueeze(0)
            probabilities, logs = agent.actor.distribution(state)
            expected = float(policy_entropy(probabilities, logs).item())
        agent.observe(transition(action=0, value=0.2))
        self.assertAlmostEqual(agent.replay.buffer[2].old_entropy, expected, places=7)
        self.assertLess(agent.replay.buffer[2].old_entropy, math.log(3.0))

    def test_one_update_per_transition_and_hard_target_sync(self):
        agent = SDSACAgent(4, 3, small_config(), torch.device("cpu"), 12)
        syncs = []
        for step in range(8):
    ...
    def test_optimize_reports_complete_finite_sac_diagnostics(self):
        agent = SDSACAgent(4, 3, small_config(), torch.device("cpu"), 13)
        ...
        self.assertAlmostEqual(diagnostics["entropy_penalty_beta"], 0.5)
        self.assertAlmostEqual(diagnostics["q_clip_range"], 0.5)
        self.assertIs(type(diagnostics["batch_size"]), int)
        self.assertIs(type(diagnostics["replay_size"]), int)
        self.assertAlmostEqual(diagnostics["actor_loss"], diagnostics["actor_base_loss"] + diagnostics["entropy_penalty_loss"], places=6)
    ...
    def test_full_checkpoint_restores_all_networks_optimizers_replay_and_rng(self):
        config = small_config()
        original = SDSACAgent(4, 3, config, torch.device("cpu"), 14)
        for step in range(7):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            restored = SDSACAgent(4, 3, config, torch.device("cpu"), 99)
    ...
    def test_policy_checkpoint_supports_two_separate_reproducible_modes(self):
        agent = SDSACAgent(4, 3, small_config(), torch.device("cpu"), 15)
        with torch.no_grad():
            for parameter in agent.actor.parameters():
                parameter.zero_()
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            policy = SDSACPolicy(path, torch.device("cpu"))
    ...
    def test_checkpoint_rejects_foreign_algorithm_and_incomplete_full_state(self):
        agent = SDSACAgent(4, 3, small_config(), torch.device("cpu"), 16)
        with tempfile.TemporaryDirectory() as directory:
            path = os.path.join(directory, "policy.pt")
            agent.save(path, kind="policy_only")
            payload = _torch_load(path, torch.device("cpu"))
            foreign = copy.deepcopy(payload); foreign["algorithm"] = "RainbowDQN"
            with self.assertRaisesRegex(ValueError, "not SDSAC"):
    ...
    def test_configuration_and_observation_fail_closed(self):
        invalid = (
            dict(alpha=0.0), dict(entropy_penalty_beta=0.0), dict(q_clip_range=0.0),
            dict(actor_learning_rate=0.0), dict(critic_learning_rate=0.0),
        ...
        agent = SDSACAgent(4, 3, small_config(), torch.device("cpu"), 17)
