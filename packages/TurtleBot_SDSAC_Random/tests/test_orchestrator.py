...
class TrainingLoopTests(unittest.TestCase):
    ...
    def test_updates_start_after_warmup_and_log_all_sdsac_diagnostics(self):
        ...
        for u in updates:
            for field in (
                ...
                "next_policy_entropy_mean", "alpha", "entropy_penalty_beta", "q_clip_range",
                "q1_clipped_mean", "q2_clipped_mean", "q1_clip_activation_fraction", "q2_clip_activation_fraction",
                "actor_grad_norm", "critic1_grad_norm", "critic2_grad_norm",
                "actor_learning_rate", "critic_learning_rate",
            ):
                self.assertNotEqual(u[field], "", field)
        self.assertTrue(all(float(u["alpha"]) == 0.2 for u in updates))
        self.assertTrue(all(float(u["entropy_penalty_beta"]) == 0.5 for u in updates))
        self.assertTrue(all(float(u["q_clip_range"]) == 0.5 for u in updates))
...
class Tier2Tests(unittest.TestCase):
    def test_post_hoc_evaluation_covers_all_scenarios_and_e3(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(sum(r["condition"] == "E2" for r in rows), 24)
            self.assertEqual(sum(r["condition"] == "E3" for r in rows), 4)
            for mode in ("stochastic", "deterministic"):
                self.assertEqual(
                    sorted(r["scenario_id"] for r in rows if r["condition"] == "E2" and r["policy_mode"] == mode),
                    sorted(s.scenario_id for s in e.scenarios),
                )
