...
class ArcScriptContractTests(unittest.TestCase):
    def test_sdsac_identity_and_entry_points_are_standalone(self):
        train_array = source("arc/sdsac_array.sbatch")
        eval_array = source("arc/sdsac_eval_array.sbatch")
        train_wrapper = source("arc/run_sdsac_seed.sh")
        preflight = source("scripts/preflight.sh")
        container = source("apptainer/tb3_phase1_foxy.def")
        self.assertIn('algorithm="SDSAC"', train_wrapper)
        self.assertIn("/sdsac/seed_", train_array)
        self.assertIn("/sdsac/seed_", eval_array)
        self.assertIn("run_sdsac_seed.sh", train_array)
        self.assertIn("run_sdsac_eval.sh", eval_array)
        self.assertIn("TurtleBot_SDSAC_Random", container)
        self.assertIn("Algorithm SDSAC", container)
        self.assertNotIn("phase1_dqn.yaml", preflight)
        self.assertNotIn("phase1_discretesac.yaml", preflight)
        self.assertIn("must not contain whitespace under ROS 2 Foxy", source("arc/run_sdsac_eval.sh"))

    def test_arrays_are_exclusive_cleanenv_and_forward_slurm_identity(self):
        for relative in ("arc/sdsac_array.sbatch", "arc/sdsac_eval_array.sbatch"):
    ...
    def test_every_arc_runtime_path_suppresses_source_bytecode(self):
        for relative in (
            ...
            self.assertIn("PYTHONDONTWRITEBYTECODE=1", source(relative), relative)
        for relative in ("arc/sdsac_array.sbatch", "arc/sdsac_eval_array.sbatch"):
            text = source(relative)
            self.assertIn("APPTAINERENV_PYTHONDONTWRITEBYTECODE=1", text, relative)
            self.assertLess(
                text.index("APPTAINERENV_PYTHONDONTWRITEBYTECODE=1"),
                text.index('CQ=("$APPTAINER_CMD" exec'),
                relative,
            )

    def test_wrappers_seal_outputs_before_complete(self):
        for relative in ("arc/run_sdsac_seed.sh", "arc/run_sdsac_eval.sh"):
            text = source(relative)
        ...
        self.assertIn("scripts/select_checkpoint.py", source("arc/run_sdsac_eval.sh"))

    def test_each_slurm_task_gets_a_unique_workspace(self):
        train = source("arc/sdsac_array.sbatch")
        evaluation = source("arc/sdsac_eval_array.sbatch")
