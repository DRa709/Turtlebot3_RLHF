"""PyTorch-independent structural guards for the SD-SAC implementation."""
...
PATH = os.path.join(ROOT, "turtlebot3_drl_nav", "sdsac.py")
...
class SDSACSourceContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        with open(PATH, encoding="utf-8") as stream:
            cls.source = stream.read()
        cls.tree = ast.parse(cls.source)
        cls.agent = class_node(cls.tree, "SDSACAgent")
    ...
    def test_is_standalone_and_has_one_named_algorithm(self):
        ...
        for forbidden in (".dqn", ".doubledqn", ".duelingdoubledqn", ".rainbowdqn", ".discretesac"):
        ...
        self.assertEqual(ast.literal_eval(assignment.value), "SDSAC")
    ...
    def test_exact_soft_value_averages_double_q_and_includes_entropy(self):
        source = segment(self.source, function_node(self.tree, "soft_value"))
        self.assertIn("average_q(q1, q2)", source)
    ...
    def test_actor_objective_sums_all_actions_without_sampled_log_prob(self):
        ...
        self.assertIn("entropy_change_penalty", source)

    def test_entropy_change_and_elementwise_q_clip_are_explicit(self):
        entropy = segment(self.source, function_node(self.tree, "entropy_change_penalty"))
        self.assertIn("0.5 * float(beta)", entropy)
        self.assertIn("old_entropy - current_entropy", entropy)
        q_clip = segment(self.source, function_node(self.tree, "q_clip_loss"))
        self.assertIn("torch.maximum", q_clip)
        self.assertLess(q_clip.index("torch.maximum"), q_clip.index(".mean()"))
        self.assertIn("clipped_q_value", q_clip)

    def test_replay_stores_collection_time_behavior_entropy(self):
        transition = class_node(self.tree, "ReplayTransition")
        self.assertIn("old_entropy", segment(self.source, transition))
        observe = segment(self.source, method_node(self.agent, "observe"))
        self.assertIn("math.log(float(self.action_dim))", observe)
        self.assertIn("policy_entropy", observe)
    ...
    def test_evaluation_requires_explicit_mode_and_seeded_stochastic_sampling(self):
        policy = class_node(self.tree, "SDSACPolicy")
