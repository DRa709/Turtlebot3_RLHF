...
class RecorderTests(unittest.TestCase):
    ...
    def test_applicability_matrix_is_sdsac_only(self):
        for algorithm, fields in UPDATE_APPLICABILITY.items():
            self.assertTrue(set(fields) <= set(UPDATES_COLUMNS), algorithm)
        self.assertEqual(set(UPDATE_APPLICABILITY), {"SDSAC"})
        self.assertEqual(set(UPDATE_APPLICABILITY["SDSAC"]), set(UPDATES_COLUMNS))
        for required in ("actor_loss", "critic1_loss", "critic2_loss", "alpha", "policy_entropy_mean",
                         "entropy_penalty_loss", "q_clip_range"):
            self.assertIn(required, UPDATE_APPLICABILITY["SDSAC"])
