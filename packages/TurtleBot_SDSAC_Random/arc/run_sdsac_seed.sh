#!/usr/bin/env bash
# ARC training wrapper for one (algorithm=SDSAC, learning seed) run. Runs inside
...
# a non-zero status.
#
# Required environment (set by arc/sdsac_array.sbatch):
...
on_walltime_warning() {
  ...
  pkill -USR1 -f "lib/turtlebot3_drl_nav/sdsac_agent" 2>/dev/null || true
...
identity = build_identity(
    experiment="phase1-random-arm", algorithm="SDSAC", algorithm_version="$PACKAGE_VERSION", arm="random",
