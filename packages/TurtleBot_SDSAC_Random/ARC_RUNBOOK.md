# ARC runbook — TurtleBot_SDSAC_Random 1.0.2
...
```text
/projects/<allocation>/tb3_sdsac/
  package/TurtleBot_SDSAC_Random/
  image/sdsac_random_v1.0.2.sif
  results/
```
...
export SDSAC_ROOT=/projects/<allocation>/tb3_sdsac
export TB3_REPO="$SDSAC_ROOT/package/TurtleBot_SDSAC_Random"
export TB3_IMAGE="$SDSAC_ROOT/image/sdsac_random_v1.0.2.sif"
mkdir -p "$SDSAC_ROOT/image"
cd "$TB3_REPO"
...
export TB3_REPO="$SDSAC_ROOT/package/TurtleBot_SDSAC_Random"
export TB3_IMAGE="$SDSAC_ROOT/image/sdsac_random_v1.0.2.sif"
export TB3_OUTPUT="$SDSAC_ROOT/results"
cd "$TB3_REPO"
```
...
sbatch --account=<acct> --partition=<part> \
  ...
  --array=0 arc/sdsac_array.sbatch
```
...
sbatch --account=<acct> --partition=<part> \
  ...
  --array=0 arc/sdsac_eval_array.sbatch
```
...
sbatch --account=<acct> --partition=<part> \
  ...
  --array=0-1 arc/sdsac_array.sbatch
```
...
sbatch --account=<acct> --partition=<part> \
  ...
  --array=0-1 arc/sdsac_eval_array.sbatch
```
...
sbatch --account=<acct> --partition=<part> \
  ...
  --array=0-4 arc/sdsac_array.sbatch
```
...
```bash
tail -f "$TB3_OUTPUT/controlled/sdsac/seed_101/job_<id>/logs/training_console.log"
python3 -m json.tool \
  "$TB3_OUTPUT/controlled/sdsac/seed_101/job_<id>/progress.json"
```
...
for STEP in 100000 200000 300000 400000 500000; do
  sbatch --account=<acct> --partition=<part> \
    ...
    --array=0-4 arc/sdsac_eval_array.sbatch
done
```
...
argmax actions (240 episodes total). The wrapper rejects an unsealed training run, unexpected
...
apptainer exec --cleanenv \
  ...
  "$TB3_IMAGE" bash -c '
    ...
    python3 scripts/make_tables.py \
      --results '"$TB3_OUTPUT"'/controlled/sdsac \
