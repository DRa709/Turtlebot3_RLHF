# Verification scope — SD-SAC random package 1.0.2
...
1. **Algorithm:** categorical actor; two online and two target critics; exact
   five-action expectations; double-average Q; behavior entropy stored in
   replay; entropy-change penalty with `beta=0.5`; elementwise Q-clip with
   `c=0.5`; fixed `alpha=0.2`; one-step terminal-only target mask; uniform replay.
...
9. **Recording:** five typed streams and SD-SAC-specific per-update diagnostics;
...
11. **Standalone analysis:** only SD-SAC identities; policy channels never
